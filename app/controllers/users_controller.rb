...
before_action :admin_only, only: [:edit, :update, :index]  
  ...
  def show
    ...
    @tankas = Tanka.joins(:dai).where(dais: {fase: 3}).where(user_id: @user.id).order(created_at: :desc)
  ...
  def index
    ...
    @users = @users_all.order(current_sign_in_at: :desc).page(params[:page]).per(5)
