...
before_action :voter_judge!, only: [:vote] 
  ...
  if params[:commit] == "投稿を締め切る" or params[:commit] == "入選歌を確定し、投稿を締め切る"
    ...
    @notice = Notice.new
    @notice.body = "「#{view_context.shorten(@dai.title,15)}」の投稿が締め切られました"
  ...
  if params[:commit] == "投票を締め切る"
    ...
    @notice = Notice.new
    @notice.body = "「#{view_context.shorten(@dai.title,15)}」の投票が締め切られました"
    @notice.link = "/dais/#{@dai.id}"
    @notice.save
    @users = User.where('id in (?)', Tanka.where('dai_id = ?', @dai.id).pluck(:user_id))
    if @users.count > 0
  ...
  def proceed
    ...
    if @dai.fase == 2 and @dai.selecting
      ...
      @notice = Notice.new
      @notice.body = "「#{view_context.shorten(@dai.title,15)}」の撰歌が終わりました"
      @notice.link = "dais/#{@dai.id}"
      @notice.save
      @users = User.where('id in (?)', Tanka.where('dai_id = ?', @dai.id).pluck(:user_id))
      if @users.count > 0
